...
def _run_proc(cmd, cwd, active_list):
    ...
    if proc.returncode not in (0, -9, None):
        yield _sse({'type': 'error', 'text': f'Proceso terminó con código {proc.returncode}'})

CHUNK_SIZE = 500

# Columnas de la tabla domicilios (= archivo Escolaridad)
_COLS_DOMICILIOS = [
    "NO_EMPLEADO", "HR_ID_PERSONA", "POSITION_NBR", "NOMBRE_COMPLETO",
    "RFC", "CURP", "PUESTO_ESTRUCTURAL", "PUESTO_FUNCIONAL", "PUESTO",
    "ESCOLARIDAD_TIPO", "ESCOLARIDAD_NIVRL", "ESCOLARIDAD_AREA", "CARRERA",
    "CENTRO_ESCOLAR", "HUMANOS_STATUS", "ESTATUS_NOMINA", "PHONE", "PHONE1",
    "CALLE", "HR_NUMERO_EXTERIOR", "HR_NUMERO_INTERIOR", "POSTAL", "COLONIA",
    "HR_MUNICIPIO", "ESTADO", "EMAIL_ADDR2", "EMAIL_ADDR", "DEPTID",
...
def _deduplicar_tabla_interno(tabla, logs):
    ...
    try:
        ...
        eliminados = fila.get('eliminados', 0)
        total_final = fila.get('total_final', 0)
        logs.append(f'  Dedup {tabla}: {eliminados} eliminados, {total_final} en tabla.')
...
def ejecutar_descarga(request):
    ...
    def stream():
        try:
            ...
            yield _sse({'type': 'log', 'text': f'Modo: {modo_tag} — scripts en {scripts_dir}'})

            for file_id in ids:
                if file_id == 6:
                    for script_name in ['consultas.js', 'descargarExcel.js']:
                        yield _sse({'type': 'log', 'text': f'Ejecutando {script_name}...'})
                    ...
                    yield _sse({'type': 'log', 'text': f'Descargando archivo ID {file_id}...'})
                    yield from _run_proc(
            ...
            if detectar_corregir:
                if os.path.exists(CORREGIR_EXE):
                    ...
                    for csv_file in csv_files:
                        ...
                        output_path = os.path.join(output_dir, csv_file.replace('.csv', '_Corregido.csv'))
                        yield _sse({'type': 'log', 'text': f'Corrigiendo: {csv_file}'})
                        yield from _run_proc(
                            [CORREGIR_EXE, input_path, '--corregir', '--salida', output_path],
                            scripts_dir,
                            _active_procs[session_id],
                        )
                else:
                    yield _sse({'type': 'error', 'text': 'AVISO: corrector no disponible — se omite.'})

            bd_stats = []
            if subir_bd_flag:
                yield _sse({'type': 'log', 'text': 'Iniciando carga a base de datos...'})
                ...
                for line in logs_a:
                    t = 'error' if line.upper().startswith(('ERROR', 'AVISO')) else 'log'
                    yield _sse({'type': t, 'text': line})

                # ── Regla B — MOV_TOTAL por chunks (streaming) ───────────────
                ruta_xlsx, engine_xlsx = _buscar_excel(temp_dir)
                if ruta_xlsx:
                    yield _sse({'type': 'log', 'text': f'Regla B - {os.path.basename(ruta_xlsx)} (engine: {engine_xlsx})'})
                    try:
                        df_b = pd.read_excel(ruta_xlsx, header=1, dtype=str, engine=engine_xlsx)
                        df_b = df_b.fillna("")
                        total_b = len(df_b)
                        yield _sse({'type': 'log', 'text': f'  Filas a procesar: {total_b}'})
                        ...
                        try:
                            ...
                            if año:
                                cur_b.execute('DELETE FROM MOV_TOTAL WHERE AÑO = %s', (año,))
                                conn_b.commit()
                                yield _sse({'type': 'log', 'text': f'  Registros del año {año} eliminados: {cur_b.rowcount} fila(s).'})
                            else:
                                yield _sse({'type': 'error', 'text': '  AVISO: no se especificó año — se omite la limpieza previa.'})

                            for inicio in range(0, total_b, CHUNK_SIZE):
                                n_chunk += 1
                                chunk = df_b.iloc[inicio:inicio + CHUNK_SIZE].copy()
                                lote_json = chunk.to_json(orient='records', force_ascii=False)
                                cur_b.callproc('sincronizar_tabla_mov_total_con_chunk_csv', [lote_json])
                                conn_b.commit()
                                fin = min(inicio + CHUNK_SIZE, total_b)
                                yield _sse({'type': 'log', 'text': f'  Lote {n_chunk}: filas {inicio + 1}–{fin} OK'})
                                del chunk, lote_json
                                gc.collect()
                            yield _sse({'type': 'log', 'text': f'  -> Movimientos cargados: {total_b} filas en {n_chunk} lote(s).'})
                        finally:
                            cur_b.close()
                            conn_b.close()
                        ...
                        yield _sse({'type': 'log', 'text': f'  Dedup MOV_TOTAL: {dedup_b.get("eliminados", 0)} eliminados, {dedup_b.get("total_final", 0)} en tabla.'})
                        bd_stats.append({
                ...
                ruta_fam = _resolver(temp_dir, 'zafiro_info_Familiares.csv', 'zafiro_info_Familiares_Corregido.csv')
                if ruta_fam:
                    yield _sse({'type': 'log', 'text': f'Regla C - {os.path.basename(ruta_fam)}'})
                    try:
                        df_c = _leer_csv(ruta_fam)
                        df_c = df_c.fillna("")
                        df_c = df_c.rename(columns={'NUM_EXTERIOR': 'CIUDAD_1', 'NUMER_INTERIOR': 'CIUDAD_2'})
                        total_c = len(df_c)
                        yield _sse({'type': 'log', 'text': f'  Filas a procesar: {total_c}'})
                        ...
                        try:
                            for inicio in range(0, total_c, CHUNK_SIZE):
                                ...
                                lote_json = chunk.to_json(orient='records', force_ascii=False)
                                cur_c.callproc('sincronizar_tabla_familiar_con_chunk_csv', [lote_json])
                                conn_c.commit()
                                fin = min(inicio + CHUNK_SIZE, total_c)
                                yield _sse({'type': 'log', 'text': f'  Lote {n_chunk}: filas {inicio + 1}–{fin} OK'})
                                del chunk, lote_json
                                gc.collect()
                            yield _sse({'type': 'log', 'text': f'  -> Familiares cargados: {total_c} filas en {n_chunk} lote(s).'})
                        finally:
                            cur_c.close()
                            conn_c.close()
                        ...
                        yield _sse({'type': 'log', 'text': f'  Dedup FAMILIAR: {dedup_c.get("eliminados", 0)} eliminados, {dedup_c.get("total_final", 0)} en tabla.'})
                        bd_stats.append({
                            'label': 'Familiares',
                            'tabla': 'FAMILIAR',
                            'insertadas': total_c,
                ...
                if ruta_esc:
                    yield _sse({'type': 'log', 'text': f'Regla D - {os.path.basename(ruta_esc)}'})
                    try:
                        df_d = _leer_csv(ruta_esc)
                        df_d = df_d.fillna("")
                        total_d = len(df_d)
                        yield _sse({'type': 'log', 'text': f'  Filas a procesar: {total_d}'})
                        ...
                        try:
                            ...
                            conn_d.commit()
                            yield _sse({'type': 'log', 'text': '  Tabla domicilios truncada.'})
                            ...
                            for inicio in range(0, total_d, CHUNK_SIZE):
                                ...
                                yield _sse({'type': 'log', 'text': f'  Lote {n_chunk}: filas {inicio + 1}–{fin} OK'})
                                gc.collect()
                            yield _sse({'type': 'log', 'text': f'  -> Escolaridad cargada: {total_d} filas en {n_chunk} lote(s).'})
                        finally:
                            cur_d.close()
                            conn_d.close()
                        ...
                        yield _sse({'type': 'log', 'text': f'  Dedup domicilios: {dedup_d.get("eliminados", 0)} eliminados, {dedup_d.get("total_final", 0)} en tabla.'})
                        bd_stats.append({
                            'label': 'Escolaridad',
                            'tabla': 'domicilios',
                            'insertadas': total_d,
                ...
                if ruta_pos:
                    yield _sse({'type': 'log', 'text': f'Regla E - {os.path.basename(ruta_pos)}'})
                    try:
                        df_e = _leer_csv(ruta_pos)
                        df_e = df_e.fillna("")
                        total_e = len(df_e)
                        yield _sse({'type': 'log', 'text': f'  Filas a procesar: {total_e}'})
                        ...
                        try:
                            ...
                            conn_e.commit()
                            yield _sse({'type': 'log', 'text': '  Tabla MOV_POS truncada.'})
                            ...
                            for inicio in range(0, total_e, CHUNK_SIZE):
                                ...
                                yield _sse({'type': 'log', 'text': f'  Lote {n_chunk}: filas {inicio + 1}–{fin} OK'})
                                gc.collect()
                            yield _sse({'type': 'log', 'text': f'  -> Posiciones cargadas: {total_e} filas en {n_chunk} lote(s).'})
                        finally:
                            cur_e.close()
                            conn_e.close()
                        ...
                        yield _sse({'type': 'log', 'text': f'  Dedup MOV_POS: {dedup_e.get("eliminados", 0)} eliminados, {dedup_e.get("total_final", 0)} en tabla.'})
                        bd_stats.append({
                            'label': 'Posiciones',
                            'tabla': 'MOV_POS',
                            'insertadas': total_e,
            ...
            yield _sse({'type': 'done', 'sessionId': session_id, 'archivos': archivos, 'bdStats': bd_stats})
